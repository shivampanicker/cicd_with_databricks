...
expected_output = spark.createDataFrame(
    ...
    schema=["product_id", "num_sales", "num_customers", "revenue", "num_returns"]
)
...
def test_aggregations():
  ...
  agg1 = sample_data.groupBy('category').agg(sum('revenue').alias('total_revenue'))
  agg2 = sample_data.groupBy('category').agg(avg('revenue').alias('average_revenue'))
