...
customer_df = spark.read.format("delta").option("header", True).load(input_path+"bronze_"+dbutils.widgets.get("source_dataset"))
...
def test_scd2_transform():
  ...
  customer_dim_df = spark.sql(f"select * from {user}_silver_db.silver_customers_test")
