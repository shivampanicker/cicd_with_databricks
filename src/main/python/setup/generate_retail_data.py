...
def generate_orders_data(num_rows: int) -> None:
    ...
    orders_df.coalesce(1).write.format("csv").option(
        "header", "true").mode("overwrite").save(output_dir+"orders")
...
def generate_sales_data(num_rows):
    ...
    for i in range(num_rows):
        ...
        sales_data.append(
            (sale_id, product_id, sale_date, sale_amount, currency))
    ...
    sales_df.coalesce(1).write.format("csv").option(
        "header", "true").mode("overwrite").save(output_dir+"sales")
...
def generate_product_data(num_rows):
    ...
    for i in range(num_rows):
        product_id = random.randint(1, 500)
        product_description = fake.random_element(elements=(
            'Entertainment', 'Gambling', 'Health', 'Mortgage & Rent Repayments', 'Sports', 'Recreation', 'Miscellaneous', 'Scientific'))
        product_start_date = fake.date_time_between(
            start_date='-5y', end_date='now')
        product_data.append(
            (product_id, product_description, product_start_date))

    # create spark dataframe
    product_df = spark.createDataFrame(product_data, schema=schema)

    product_df.coalesce(1).write.format("csv").option(
        "header", "true").mode("overwrite").save(output_dir+"products")
...
def generate_customer_data_day_0(num_rows: int):
    ...
    customer_df.coalesce(1).write.format("csv").option(
        "header", "true").mode("overwrite").save(output_dir+"customers/")
...
def generate_customer_data_day_2():
    ...
    for i in range(3000):
        ...
        customer_data.append(
            (customer_id, customer_name, state, company, phone_number))
