...
def generate_orders_data(num_rows: int) -> None:
    ...
    orders_df.coalesce(1).write.format("csv").option("header", "true").mode("overwrite").save(output_dir+"orders")
...
def generate_sales_data(num_rows):
  ...
  sales_df.coalesce(1).write.format("csv").option("header", "true").mode("overwrite").save(output_dir+"sales")
...
def generate_product_data(num_rows):
    ...
    for i in range(num_rows):
      product_id = random.randint(1, 500)
      product_description = fake.random_element(elements=('Entertainment', 'Gambling', 'Health', 'Mortgage & Rent Repayments', 'Sports', 'Recreation', 'Miscellaneous', 'Scientific'))
    ...
    product_df.coalesce(1).write.format("csv").option("header", "true").mode("overwrite").save(output_dir+"products")
...
def generate_customer_data_day_0(num_rows: int):
  ...
  customer_df.coalesce(1).write.format("csv").option("header","true").mode("overwrite").save(output_dir+"customers/")
...
def generate_customer_data_day_2():
  ...
  # Write to Delta Lake as bronze layer
  customer_df.coalesce(1).write.format("csv").option("header","true").mode("append").save(output_dir+"customers")
